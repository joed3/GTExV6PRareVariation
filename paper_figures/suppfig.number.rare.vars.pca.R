...
subjectFile = '/mnt/lab_data/montgomery/shared/datasets/gtex/GTEx_Analysis_2015-01-12/sample_annotations/GTEx_Analysis_2015-01-12_\
Annotations_SubjectPhenotypesDS.txt' # adjust path as necessary
...
font.sizing = theme(text = element_text(size = 14),
...
counthist = ggplot(varCounts.melted, aes(x = value, fill = variable, colour = 'white')) + geom_histogram() +
    ...
    scale_fill_manual(values = colours) + scale_colour_manual(values = 'white') +
        theme(strip.background = element_blank()) + font.sizing
...
combined.plot = ggdraw() +
    draw_plot(counthist, 0.02, 0.8, 0.98, 0.2) +
    draw_plot(pca, 0, 0.25, 1, 0.55) +
    draw_plot(pca.subset, 0.48, 0.3, 0.47, 0.35) +
    draw_plot(count.ratio.plot.withPC.subset, 0, 0, 1, 0.25) +
    draw_plot_label(c('a','b','c'), c(0,0,0), c(1,0.8, 0.25), size = 16)


pdf(paste0(dir, '/paper_figures/suppfig.varcounts.pca.pdf'), height = 12, width = 10.5)
