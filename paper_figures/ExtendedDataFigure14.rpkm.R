...
#
# output:
#       1. ExtendedDataFigure14.rpkm.i.pdf (a, b separately)
#       2. ExtendedDataFigure14.i.tissue_order.txt: order of tissues in figures (a, b separately) generated
#          by running ExtendedDataFigure14.Zscores.R
#       3. ExtendedDataFigure14.rpkm.out.RData
...
colnames(clinvar.rpkm)[10] <- "AbsMed"

# load RPKM values of genes for all individuals
clinvar.rpkm_bg = read.table(paste(dir,"/data/rpkm.ClinVar.all.txt",sep=""),
                               sep='\t', header = TRUE, na.strings = "NA",check.names=FALSE)
...
idx_samples = which(clinvar.rpkm[,"gene"]==target_gene)

# list of observed tissues for the selected samples
obs_tissues = colnames(clinvar.rpkm)[setdiff(which(apply(!is.na(clinvar.rpkm[idx_samples,]),2,sum,na.rm=TRUE)>0),seq(1,10,1))]

idx_indiv = which(clinvar.rpkm_bg[,"gene"] == target_gene)
exp_mat = data.frame(clinvar.rpkm_bg[idx_indiv,4:ncol(clinvar.rpkm_bg)],
                     MedZ=matrix(NA,length(idx_indiv),1),check.names=FALSE) # 44 tissues + 1 median
exp_mat[,"AbsMed"] = apply(exp_mat[,1:ncol(exp_mat)-1],1,median,na.rm=TRUE)

obs_tissues_sorted = as.character(unlist(read.table(paste(dir,"/data/ExtendedDataFigure14.a.tissue_order.txt",sep=""), 
...
for (s in idx_samples) {
  for (i in 1:length(obs_tissues_sorted)) {
    if (is.na(clinvar.rpkm[s,obs_tissues_sorted[i]]) == FALSE) {
      names = c(names,group_name[i])
      values = c(values,clinvar.rpkm[s,obs_tissues_sorted[i]])
...
ggplot(data=exp_gene, aes(names,values,fill=names))+
  ...
  ggtitle("SBDS") + coord_flip() + theme_bw() + labs(x="Tissues",y="RPKM") + guides(fill=FALSE)+
...
ggsave(paste(dir,"/paper_figures/ExtendedDataFigure14.rpkm.a.pdf",sep=""), width=6, height=6.5)
...
obs_tissues_sorted = as.character(unlist(read.table(paste(dir,"/data/ExtendedDataFigure14.b.tissue_order.txt",sep=""), 
                                                    sep='\t', header = TRUE, na.strings = "NA")))
labels_readable = vector()
for (i in 1:length(obs_tissues_sorted)-1) {
  labels_readable = c(labels_readable,
...
ggplot(data=exp_gene, aes(names,values,fill=names))+
  ...
  scale_x_discrete(breaks=group_name[1:length(obs_tissues_sorted)],
                   labels=labels_readable)+
  scale_fill_manual(values=colors)+
  geom_point(data=data_ind[which(data_ind[,"snvs"]==list_snvs[1]),],aes(names,values),color="darkred",size=1.5,shape=8)+
  ggtitle("GAMT") + coord_flip()+theme_bw() + labs(x="Tissues",y="RPKM") + guides(fill=FALSE) +
  theme(plot.title = element_text(size = 12),
        axis.title.x = element_text(size=12),
        axis.text.x  = element_text(size=10),
        axis.title.y = element_text(size=12),
...
ggsave(paste(dir,"/paper_figures/ExtendedDataFigure14.rpkm.b.pdf",sep=""),width=6,height=2.5)



# =========== Save data
save.image(file = paste(dir,"/data/ExtendedDataFigure14.rpkm.out.RData",sep=""))
