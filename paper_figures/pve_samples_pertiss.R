# =======================================================================================================
# This is a script of 1st step for generating EDF1b.
#
# input:
...
#tiss <- 1
for (tiss in (1:44)){
  sample.covariates <- dat.samples[[tiss]][gsub('[.]','-',rownames(expr.removed[[tiss]])),]
  sample.names <- rownames(sample.covariates)
  
  ## again 0 for low values
  sample.covariates <- sample.covariates[,which(sapply(sample.covariates, function(y) sum(table(unique(y)))>1))]
  sample.covariates <- sample.covariates[,which(sapply(sample.covariates, function(z) length(which(!is.na(z)))>=50))]
  
  
  sample.covariates <- as.data.frame(lapply(sample.covariates, function(x){
    sapply(x, function(y){
  ...
  rownames(sample.covariates) <- sample.names
  ...
  pve.function <- function(x,y){
    ...
    for(j in 1:ncol(y)){
      ...
      #       pve.vector[j] <- mean(sapply(summary(lm(x~y[,j])), function(x) x$adj.r.squared))
  ...
  pve.explained <- pve.function(expr.removed[[tiss]], sample.covariates)
  save(pve.explained, file = paste("/data/pve_samples_",tiss,".RData",sep=""))
}
